...
const CreateCollection = (props: any) => {
  ...
  function fetchCollection() {
    axios
      .get(`/collection/detail/${tempName}`)
      .then((res) => {
  ...
  const FeaturedImgChange = (e: any) => {
    if (e.target.files && e.target.files.length > 0) {
      setFeaturedImg(e.target.files[0]);
      setFeaturedUri("");
    }
  };
  const BannerImgChange = (e: any) => {
    if (e.target.files && e.target.files.length > 0) {
  ...
  const categoryHandle = (value: any) => {
    setCategory(value);
  };
  const feeHandle = (e: any) => {
    setFee(e.target.value);
  };
  const yourSiteHandle = (e: any) => {
    setYourSite(e.target.value);
  };
  const discordHandle = (e: any) => {
    setDiscord(e.target.value);
  };
  const instagramHandle = (e: any) => {
    setInstagram(e.target.value);
  };
  const mediumHandle = (e: any) => {
    setMedium(e.target.value);
  };
  const tmeHandle = (e: any) => {
  ...
  async function onCreateCollection() {
    ...
    try {
      ...
      await axios
        .post(`/collection/update/`, metaData)
        .then((res) => {
          tempName = name;
          setCollection(res.data.collection);
  ...
  return (
    <div className="createCollectionContainer">
      ...
      <Logo logoUri={logo_uri} onChange={(e: any) => logoChange(e)} />
        ...
        <div className="FeaturedImgInput">
          <label htmlFor="FeaturedInput">
            <input
              ...
              accept="image/*"
              style={{ display: "none" }}
              onChange={FeaturedImgChange}
            />
            {!FeaturedImg && featured_uri !== "" ? (
            ...
            {FeaturedImg && featured_uri === "" && (
              <div className="FeaturedImg">
                <img
                  src={URL.createObjectURL(FeaturedImg)}
                  width={300}
                  height={200}
                  alt=""
                />
      ...
      <div className={"formControl"}>
        ...
        <p>
          ...
          on different devices. 1400 x 400 recommended.recommended
        </p>
        <div className="BannerImgInput">
          <label htmlFor="BannerInput">
            <input
              ...
              accept="image/*"
              style={{ display: "none" }}
              onChange={BannerImgChange}
            />
            {!BannerImg && banner_uri !== "" ? (
              <i className="fa fa-image" style={{ fontSize: "4.5em" }} />
            ) : (
      ...
      <div className={"formControl"}>
        ...
        <textarea
          ...
          required
          value={description}
          placeholder={"Write briefly about this collection"}
        />
      </div>
      ...
      <div className={"formControl"}>
        ...
        <SelectMenu
          onChangeHandle={(val: any) => categoryHandle(val)}
          data={allCategories}
          initialState="Add categories"
        />
      ...
      <div className={"formControl"}>
        ...
        <div>
          ...
          <div
            className={"linkInput"}
            style={{ borderRadius: "0 0 6px 6px", borderBottomWidth: "1px" }}
          >
            <i className="fa fa-globe" />
      ...
      <div className={"formControl"}>
        ...
        <p>
          <a
            target={"_blank"}
            rel="noreferrer"
            href={"https://support.opensea.io/hc/en-us/articles/1500009575482"}
            style={{ color: "rgb(32, 129, 226)" }}
          >
      ...
      >
        <strong>{isCreate ? "Create" : "Save"}</strong>
      </Button>
    </div>
  );
};
